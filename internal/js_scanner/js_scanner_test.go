...
func topLevelReturnFixtures() []testcase {
	return []testcase{
		{
			name:   "basic",
			source: `return "value";`,
...
func TestGetTopLevelReturns(t *testing.T) {
	tests := topLevelReturnFixtures()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := FindTopLevelReturns([]byte(tt.source))
			got := strings.Trim(strings.Join(strings.Fields(fmt.Sprint(result)), ","), "[]")
			// compare to expected string, show diff if mismatch
			if diff := test_utils.ANSIDiff(strings.TrimSpace(tt.want), strings.TrimSpace(got)); diff != "" {
