...
describe('tsx/meta', { skip: true }, () => {
	it('return ranges', async () => {
		const input = `---\nconsole.log("Hello!")\n---\n\n<div></div>`;
		const { metaRanges } = await convertToTSX(input, { sourcemap: 'external' });

		assert.deepStrictEqual(metaRanges, {
			frontmatter: {
				start: 30,
				end: 54,
			},
	...
	it('return ranges - no frontmatter', async () => {
		const input = '<div></div>';
		const { metaRanges } = await convertToTSX(input, { sourcemap: 'external' });
	...
	it('return proper ranges with multibyte characters', async () => {
		const input = '---\n🦄\n---\n\n<div></div>';
		const { metaRanges } = await convertToTSX(input, { sourcemap: 'external' });
	...
	it('extract scripts and styles with multibyte characters', async () => {
	...
	it('extract scripts with multibyte characters II', async () => {
