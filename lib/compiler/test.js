...
`,
    {
      ...
      preprocessScript: async (value, attrs) => {
        let x = i++;
        if (!attrs.lang) {
          return null;
        }
        ...
        // await sleep(3000);
        // console.log(`Finished preprocessing ${x}`);
        return { code: value.replace('js', 'ts') };
      },
    }
  );
  // console.log(result);
  ...
  if (!result.code.includes('const js')) {
    throw new Error(`Scripts didn’t transform as expected. Expected "const js" to be present.`);
  }
